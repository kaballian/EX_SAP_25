...
\begin{itemize}
    \item Identity \& Access Management - Univention\cite{UNIVENTION}, a German based IAM solution, 
    that can handle AD, and their IAM service NUBUS\cite{NUBUS}.  
    \item Cloud compute \& Hosting - Scaleway\cite{SCALEWAY}, a French based cloud compute company, which
    ...
    hosted by Scaleway.
    \item Collaboration \& Productivity tools - LibreOffice\cite{LIBREOFFICE} is an latvian alternative to the
    Microsoft office suite. As for collaboration tools, NextCloud, which originates from Germany.
    \item Security \& Monitoring - WithSecure\cite{WITHSECURE} from Finland, An XDR solution is chosen, because the necessty of
    more than just an antivirus at enterprise level is required. Along with LogPoint\cite{LOGPOINT}, a Danish company, which is a 
    SIEM solution, which is an information correlation service.
    \item Patient/Customer identity - Univention's NUBUS has an integration with KeyCloak\cite{KEYCLOAK}, which is 
    ...
    is still in its infancy, a japanese alternative from Fujitsu\cite{SERVER_FUJITSU} could be considered.
\end{itemize}
